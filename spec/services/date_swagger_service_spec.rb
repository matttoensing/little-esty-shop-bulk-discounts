...
RSpec.describe DateSwaggerService do
  ...
  it 'returns an array' do
    expect(DateSwaggerService.get_holidays).to be_an(Array)
  end

  it 'returns the first 3 holidays' do
    expected = ["Labor Day", "Columbus Day", "Veterans Day"]

    expect(DateSwaggerService.next_three_holidays_names).to eq(expected)
  end

  it 'returns the dates of the next 3 holidays' do
    expected = ["2021-09-06", "2021-10-11", "2021-11-11"]

    expect(DateSwaggerService.next_three_holidays_dates).to eq(expected)
  end
  ...
  it 'returns a hash of holidays and dates' do
    ...
    expect(DateSwaggerService.next_three_holidays).to eq(expected)
  end
end
